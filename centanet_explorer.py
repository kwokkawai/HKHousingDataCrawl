...
async def explore_centanet_structure():
    """探索中原地产页面结构"""
    print("="*70)
    print("探索中原地产页面结构")
    print("="*70)
    ...
    browser_config = BrowserConfig(
        headless=True,
        user_agent=CENTANET_CONFIG.user_agent,
    )
    
    async with AsyncWebCrawler(config=browser_config) as crawler:
        # 爬取列表页
        print(f"\n正在访问列表页: {CENTANET_CONFIG.list_url}")
        result = await crawler.arun(
            url=CENTANET_CONFIG.list_url,
            timeout=CENTANET_CONFIG.timeout
        )
        ...
        html_file = output_dir / "centanet_list_page.html"
        with open(html_file, 'w', encoding='utf-8') as f:
            f.write(result.html)
        print(f"  HTML已保存到: {html_file}")
        
        # 保存Markdown用于分析
        md_file = output_dir / "centanet_list_page.md"
        ...
        try:
                ...
                for pattern in link_patterns:
                    if pattern in href.lower():
                        if not href.startswith('http'):
                            href = CENTANET_CONFIG.base_url + href
            ...
            summary_file = output_dir / "centanet_analysis_summary.json"
        ...
        print("3. 运行 centanet_crawler.py 进行实际爬取")

if __name__ == "__main__":
    asyncio.run(explore_centanet_structure())
